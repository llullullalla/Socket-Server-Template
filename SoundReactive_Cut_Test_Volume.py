...
def play_sound(file_path):
    ...
    pygame.mixer.music.stop()  # Ensure no overlap
...
# Optional frame skipping: Process every nth frame
frame_skip_rate = 5
frame_counter = 0
...
while True:
    ...
    # Play sound if count changes and delay has passed
    if current_people_count != previous_people_count and (datetime.datetime.now() - last_sound_change > datetime.timedelta(milliseconds=500)):
        if current_people_count == 0:
            ...
            # Check if audio file exists before attempting to play it
            selected_audio_file = audio_files.get(min(current_people_count, 5), audio_files[5])
            if file_exists(selected_audio_file):
                play_sound(selected_audio_file)
